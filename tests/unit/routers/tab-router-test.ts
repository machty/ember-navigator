...
import { route, stackRouter, tabRouter } from 'ember-navigator';
...
module('Unit - TabRouter test', function (hooks) {
  ...
  test('initial state', function (assert) {
    let router = tabRouter('root', [route('foo'), route('bar')]);
  ...
  test('navigating between shallow routes', function (assert) {
    ...
    assert.strictEqual(innerRoute.index, 1);
    assert.deepEqual(
      innerRoute.routes.map((r) => r.routeName),
      ['b1', 'b2']
    );
  });

  test("navigating to the parent route if a route you're in should be a no-op", function (assert) {
    let router = tabRouter('a', [stackRouter('b', [route('c')])]);

    let initialState = router.getInitialState();
    let state2 = navigate(router, initialState, 'b');

    assert.strictEqual(initialState, state2);
  });

  test('navigating away', function (assert) {
    let router = tabRouter('root', [route('a'), route('b')]);
    ...
    assert.strictEqual(state3.index, 0);
  });
  ...
  test('navigation within active route', function (assert) {
    ...
    assert.strictEqual(state2.index, 0);

    let newNestedRoute = state2.routes[0] as RouterState;

    assert.strictEqual(newNestedRoute.index, 1);
  });
  ...
  test('navigation to another tab route preserves the state of each tab', function (assert) {
    ...
    assert.strictEqual(
      tabRouterState.routes.length,
      2,
      'first tab should still be drilled down one level'
    );
    assert.strictEqual(
      tabRouterState.routes[1].routeName,
      'a2',
      'first tab should still have a2 active'
    );
  });
});
...
function buildExampleRouter() {
  return tabRouter('root', [
    stackRouter('a', [route('a1'), route('a2')]),
    stackRouter('b', [route('b1'), route('b2')]),
    stackRouter('c', [route('c1'), route('c2')]),
    route('shallow'),
  ]);
}
